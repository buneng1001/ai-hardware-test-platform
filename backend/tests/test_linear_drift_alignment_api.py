...
def test_linear_drift_alignment_estimates_multiple_event_model_and_improves_residuals(tmp_path, monkeypatch):
    ...
    assert (
        alignment["post_alignment"]["camera_4"]["max_residual_ms"]
...
def test_alignment_review_rejects_excluding_too_many_common_anchors(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        run = _wait_for_completion(client, client.post(f"/api/collection-tasks/{task['id']}/runs").json()["id"])
        anchors = [
            anchor["id"] for anchor in run["alignment_result"]["anchor_details"] if anchor["channel"] == "camera_1"
        ]
        response = client.post(
            f"/api/runs/{run['id']}/alignment-review",
